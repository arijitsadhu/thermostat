...
static void ntp_request(NTP_T* state)
{
    ...
    // Note that when using pico_cyw_arch_poll these calls are a no-op and can be
...
void ntp_client_poll(void)
{
    ...
    if (state) {
        if (absolute_time_diff_us(get_absolute_time(), state->ntp_test_time) < 0 && !state->dns_request_sent) {
            ...
            // from lwIP. Note that when using pico_cyw_arch_poll these calls are
            // a no-op and can be omitted, but it is a good practice to use them
            // in case you switch the cyw43_arch type later.
            cyw43_arch_lwip_begin();
